...
本项目使用 AGPL-3.0 许可证开源，并存在以下例外情况：
+ 你**可以**引用这个项目为依赖，而无需使用相同许可证开源你的项目
+ 你**可以**通过 shadow/shade 等方式，将这个项目作为依赖打包到你的项目中，而无需使用相同许可证开源你的项目
+ 你**可以**将本项目**以合适的方式**用于商业用途，简单来说，拿来实现一些功能是欢迎的，投机取巧（尤其是损害作者利益）是禁止的
+ 你**不被允许**以任何方式分发 `plugin` 模块的构建产物
+ 有任何使用本项目的情况，我们都**建议**你声明本项目的地址，以支持作者的开发工作

> 总结：开源付费的分发形式已经相当宽容，只要不损害我的利益，你可以获得相对较高的自由度。
...
通过购买获得的插件与开源仓库构建完全一致，从 `1.0.0` 版本开始到最新版本都是一样，没有任何区别，仅当做对我的支持。
